...
9. **Archive the run's transcripts to S3 (best-effort, opt-in).** This step runs only when the environment variable `ORCHESTRATOR_TRANSCRIPT_BUCKET` is set and non-empty (check with `printenv ORCHESTRATOR_TRANSCRIPT_BUCKET`); if it is unset or empty, **skip the archive** — record "transcript archive: not configured" for the issue's `Transcript archive` section and continue to step 10. When set, upload the conversation transcript JSONL of every agent involved in the run to that bucket — `s3://$ORCHESTRATOR_TRANSCRIPT_BUCKET`, one object per agent, key `<repository-name>/<orchestration-log-filename-without-.md>/<agent-name>.jsonl` — where `<repository-name>` is the repository's name on GitHub (e.g. `gh repo view --json name -q .name`) and the middle segment is the orchestration log's filename stripped of its `.md` extension:
   ...
   2. Upload the teammates first: `aws s3 cp <transcript> s3://$ORCHESTRATOR_TRANSCRIPT_BUCKET/<repository-name>/<log-name>/developer.jsonl`, and likewise for `reviewer` (and `reviewer-2`, if a replacement was spawned).
